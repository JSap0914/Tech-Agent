...
class RedisClient:
    ...
    async def get_code_analysis(self, file_hash: str) -> Optional[dict]:
        """
        Get cached code analysis results.

        Args:
            file_hash: Hash of the code file

        Returns:
            Cached analysis or None
        """
        key = f"code_analysis:{file_hash}"
        return await self.get(key)

    async def set_code_analysis(
        self,
        file_hash: str,
        ...
        """
        key = f"code_analysis:{file_hash}"
        return await self.set(key, analysis_results, ttl)

    async def get_api_inference(self, project_id: str) -> Optional[dict]:
        """
        Get cached API inference results.

        Args:
            project_id: Project identifier

        Returns:
            Cached API spec or None
        """
        key = f"api_inference:{project_id}"
        return await self.get(key)

    async def set_api_inference(
        self,
        project_id: str,
        api_spec: dict,
        ttl: Optional[int] = 7200  # 2 hours default
    ) -> bool:
        """
        Cache API inference results.

        Args:
            project_id: Project identifier
            api_spec: API specification to cache
            ttl: Cache TTL in seconds
        ...
        """
        key = f"api_inference:{project_id}"
        return await self.set(key, api_spec, ttl)
